...
import OgretmenSinavYonetimi from './Screens/OgretmenSinavYonetimi';
  ...
  return (
    <SafeAreaView style={[styles.drawerContainer, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.drawerHeader, { borderBottomColor: theme.colors.border }]}>
        ...
        <Text style={[styles.appVersion, { color: theme.colors.textSecondary }]}>Mobil Uygulama</Text>
      </View>
...
function DrawerRoutes() {
  ...
  const [userType, setUserType] = useState('student'); // Varsayılan olarak öğrenci
  ...
  useEffect(() => {
    ...
    const checkUserType = async () => {
      try {
        ...
        console.error('Kullanıcı tipi kontrol edilirken hata oluştu:', error);
      }
    };
    checkUserType();
  }, []);
  ...
  return (
    <Drawer.Navigator
      ...
      <Drawer.Screen name="Duyurular" component={Duyurular} options={{
        drawerIcon: ({ color }) => <Ionicons name="megaphone-outline" size={22} color={color} />,
      }} />
      {userType === 'teacher' && (
        <Drawer.Screen 
          name="OgretmenSinavYonetimi" 
          component={OgretmenSinavYonetimi} 
          options={{
            drawerLabel: 'Sınav Yönetimi',
    ...
    <ThemeProvider>
      <NavigationContainer>
        <Stack.Navigator initialRouteName="Login">
          ...
          <Stack.Screen name="DuyuruDetay" component={DuyuruDetay} options={{ headerShown: false }} />
        </Stack.Navigator>
      </NavigationContainer>
    </ThemeProvider>
  );
